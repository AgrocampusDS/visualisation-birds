...
```{r warning=FALSE}
colors<-c(aquatique, insessorial, aerien, terrestre, generaliste)

b1 <- species.traits %>% 
  ggplot(aes(x=Lifestyle,y=lm(log(Wing.Length)~log(Mass))$resid,fill=Lifestyle))+
  geom_boxplot()+
  facet_wrap(~Generalist_specialist, scales = "free", ncol=1)+
...
b3 <- species.traits %>% 
  ggplot(aes(x=Lifestyle,y=lm(log(Tail.Length)~log(Mass))$resid,fill=Lifestyle))+
  geom_boxplot()+
  facet_grid(~Generalist_specialist, scales = "free_y", space = "free_y", cols=1)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red")+
  coord_flip()+
  theme_ridges() +
  ylim(-0.5, 0.5)+
...
p1 <- species.traits %>% filter(Mass < 1000) %>% 
  ggplot()+
  aes(x=Wing.Length, y=Mass, col=Lifestyle)+
  geom_point()+
