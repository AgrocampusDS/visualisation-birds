...
g1 <- species %>% select(Species1, Mass) %>% rename(species = Species1) %>% filter(Mass < 1000) %>% 
  ...
  labs(title = "Distribution de la masse", 
       subtitle = "zoom sur toutes les espèces dont la masse est inférieure à 1kg")+
  xlab("Masse (g)")+
  ylab("Densité")+
...
insessorial <- "#285928"
terrestre <- "#664b2c"
colors<-c(aerien, aquatique, insessorial, terrestre, generaliste)

g3 <- species %>% select(Species1, Mass, Lifestyle) %>% rename(species = Species1) %>% 
...
species.traits %>% 
  ...
  # scale_fill_manual(values = cols.T) + 
  theme(axis.text.x = element_text(angle= 45,hjust=1,color="black"))+
  labs(x=NULL, y ="Taille relative de l'aile")+
  theme(legend.position='none')
```
...
species.traits %>% 
  ...
  theme_classic()+
  theme(axis.text.x = element_text(angle= 45,hjust=1,color="black"))+
  labs(x=NULL, y ="Taille relative de la queue")+
  theme(legend.position='none')
```



## Grid : en quoi les oiseaux aquatiques se démarquent ?
...
b1 <- species.traits %>% 
  ...
  theme(legend.position='none', 
        axis.text.y = element_text(colour = c("red", "black", "black", "black", "black")),
...
p1 <- species.traits %>% filter(Mass < 1000) %>% 
  ggplot()+
  aes(x=Wing.Length, y=Mass, col=class)+
  geom_point()+
  geom_smooth(method="lm")+
  scale_color_manual(values=c("#545a61", aquatique))+
  theme_ridges()+
  theme(legend.position = "none")

p2 <- species.traits %>% filter(Mass < 1000) %>% 
  ggplot()+
  aes(x= Tarsus.Length, y=Mass, col=class)+
  geom_point()+
  geom_smooth(method="lm")+
  scale_color_manual(values=c("#545a61", aquatique))+
  theme_ridges()+
...
p3 <- species.traits %>% filter(Mass < 1000) %>% 
  ggplot()+
  aes(x= Tail.Length, y=Mass, col=class)+
  geom_point()+
  geom_smooth(method = "lm")+
  scale_color_manual(values=c("#545a61", aquatique))+
  theme_ridges()+
...
p4<- species.traits %>% filter(Mass < 1000) %>% 
  ggplot()+
  aes(x= Beak.Length_Culmen, y=Mass, col=class)+
  geom_point()+
  geom_smooth(method = "lm")+
  scale_color_manual(values=c("#545a61", aquatique))+
  theme_ridges()+
