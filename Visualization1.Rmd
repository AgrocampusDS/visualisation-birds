...
```
```{r}
species.Coord %>% select(COUNTRY, geometry, Species, Habitat) %>% group_by(COUNTRY, Habitat) %>% mutate("nb_habitat" = n())



ggplot() + geom_boxplot(fill = )
...
#ggplot() + geom_sf(aes(geometry = geometry), fill=)
```
...
```{r}
for (pays in species$Country_WRI){
  print(mygeocode(pays))
}
```
...
```{r}
data_map <- species %>% rename(name_long = Country_WRI) %>% 
  left_join(world)
class(data_map)
