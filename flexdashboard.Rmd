...
```{r}
gridB.sf  %>% mutate(grp=as.factor(col)) %>% filter(masse_med < 1000) %>% ggplot() +
  aes(x = masse_med,y=..density.., fill=..density..) +
  ...
  scale_fill_gradientn(colours = colors, name = "", 
                       guide=guide_colourbar(title.position = "top", 
                                             ...
                                             label.position = "left"))+
  theme_ridges() +
  labs(title = "Densité des groupes associés à la masse", subtitle = "pour des masses inférieures à 1kg")+
  theme(plot.title = element_text(hjust = 0.2, vjust=3, size = 15, family="Montserrat", face="bold"),
        plot.subtitle = element_text(hjust = 0.15, vjust=5, family="Montserrat", face="italic"),
        ...
        legend.key.height = unit(15.5, "mm"), 
